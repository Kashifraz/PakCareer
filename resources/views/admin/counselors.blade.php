        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="relative overflow-x-auto sm:px-6 lg:px-8 ">
                <div class="flex items-center mt-4 justify-between pb-4 right-0 float-right">
                    <form action="{{route('admin.counselors')}}" method="POST" class="flex items-center mb-5">
                        @csrf
                ...
                <table class="w-full text-md mt-4 text-left text-gray-500 rounded-lg shadow ">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50">
                            <td class="px-6 py-4">
                                @php
                                $profile_image = $counselor->profile->profile_image;
                                @endphp
                                <p class="flex inline">
                                    @if ($profile_image!='NA')
                            ...
                            <td class="px-6 py-4">
                                {{$counselor->profile->profession}}
                            </td>

                            <td class="px-6 py-4">
                                <a href="{{route('delete.counselors', $counselor->id)}}" class="font-medium text-lg text-red-600  ml-3">
                                    <i class="fa-solid fa-trash"></i>
                                </a>
                                @if(!$counselor->verified)
                                <a href="{{route('verify.counselors', $counselor->id)}}" class="font-medium text-lg text-blue-600  ml-3">
                                    <i class="fa-solid fa-check"></i>
                                </a>
                                @else
                                <span class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded "><i class="fa-solid fa-check"></i> Verified</span>
                                @endif
                ...
                <div class="mt-6">{{$counselors->links()}}</div>
            </div>
